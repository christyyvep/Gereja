...
function shouldGenerateForDate(recurring, targetDate) {
  ...
  if (!pattern) {
    console.log('❌ [Debug] No recurrence pattern found for schedule:', recurring.id)
    return false
  }
  ...
  console.log('🔍 [Debug] Checking schedule generation:')
  console.log('- Target date:', targetDate)
  console.log('- Start date:', pattern.startDate)
  console.log('- Pattern type:', pattern.type)
  console.log('- Pattern interval:', pattern.interval)
  
  // Check if target date is before start date
  if (targetDateObj < startDate) {
    console.log('❌ [Debug] Target date is before start date')
    return false
  }
  
  // Check if target date is after end date (if exists)
  if (pattern.endDate && targetDateObj > new Date(pattern.endDate)) {
    console.log('❌ [Debug] Target date is after end date:', pattern.endDate)
    return false
  }
  ...
  switch (pattern.type) {
    ...
    default:
      console.log('❌ [Debug] Unknown pattern type:', pattern.type)
      return false
  }
}
